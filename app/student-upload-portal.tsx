      ...
      <div className="record-main">
        ...
        <div className="record-grid">
          ...
          <div>
            ...
            <span className={`status-chip ${uploaded ? "uploaded" : "pending"}`}>
          ...
          <div className="record-action">
            <input
              ...
              onChange={(event) => {
                ...
                if (file) onChooseFile(entry, file);
...
export default function StudentUploadPortal() {
  ...
  useEffect(() => {
    let active = true;
    fetch("/api/evidence/status", { cache: "no-store" })
      .then(async (response) => {
        ...
        if (active) setUploadedIds(new Set(payload.uploadedEntryIds ?? []));
      })
      .catch(() => {
        if (active) {
          setNotice({
  ...
  const filteredEntries = useMemo(() => {
    ...
    return studentEntries.filter((entry) => {
      const matchesName =
        !normalizedQuery ||
        entry.studentNames.toLocaleLowerCase().includes(normalizedQuery);
      const matchesYear = year === "all" || entry.academicYear === year;
  ...
  const completion = Math.round((uploadedTotal / studentEntries.length) * 100);
  ...
  function chooseFile(entry: StudentEntry, file: File) {
    if (!ACCEPTED_TYPES.includes(file.type)) {
      setNotice({
        type: "error",
        message: "Please choose a PDF, JPG, or PNG document.",
      });
      return;
    }
  ...
  async function uploadEvidence() {
    ...
    try {
      ...
      if (!presignResponse.ok || !payload.uploadUrl) {
        throw new Error(payload.error || "Upload could not be started.");
      }
      ...
      setUploadedIds((current) => new Set(current).add(entry.id));
      setNotice({
        type: "success",
    ...
    <main className="portal">
      <header className="topbar">
        <div className="topbar-inner">
          ...
          <div>
            <p className="brand-title">Institute Accreditation Portal</p>
            <p className="brand-subtitle">Electronics &amp; Communication Department</p>
          </div>
          <a className="help-link" href="#upload-guidelines">
            Upload guidelines
          </a>
        </div>
      </header>
      ...
      <div className="portal-shell">
        <section className="hero" aria-labelledby="page-title">
          <p className="eyebrow">NBA Accreditation · Criterion 4</p>
          <h1 id="page-title">Student Participation in Professional Events</h1>
          <p className="hero-copy">
            Criterion 4.7.2 · Find your fixed activity entry and upload its
            supporting evidence.
          </p>
        ...
        <form className="search-panel" onSubmit={submitSearch}>
          ...
          <label className="select-field">
            <span className="visually-hidden">Academic year</span>
            <select
              value={year}
              onChange={(event) => {
          ...
          <label className="select-field">
            <span className="visually-hidden">Evidence status</span>
            <select
              value={statusFilter}
              onChange={(event) => {
            ...
            >
              <option value="all">All evidence status</option>
        ...
        <section className="progress-panel" aria-label="Document progress">
          <div className="progress-copy">
            <div>
              <p className="progress-title">Document progress</p>
            ...
            <p className="progress-percent">{completion}% complete</p>
          </div>
          <div
        ...
        <div className="results-heading">
          <div>
            <p className="eyebrow">Fixed records</p>
        ...
        <section className="record-list" aria-label="Student activity entries">
          {visibleEntries.map((entry, index) => (
            <EntryCard
              key={entry.id}
              entry={entry}
              position={index + 1}
              uploaded={uploadedIds.has(entry.id)}
        ...
        {filteredEntries.length === 0 ? (
          <section className="empty-state">
            ...
            <p>
              Check the spelling of your name or select another academic year.
              The institutional activity details cannot be edited here.
            </p>
          </section>
        ) : null}
        ...
        <section className="guidelines" id="upload-guidelines">
          <div>
            <p className="eyebrow">Before uploading</p>
            <h2>Evidence guidelines</h2>
          </div>
          <ul>
            <li>Upload one clear PDF, JPG, or PNG file for the selected entry.</li>
            <li>The maximum allowed file size is 10 MB.</li>
            <li>
              Uploading again replaces the previous evidence for that entry.
            </li>
          </ul>
        </section>
      </div>
      ...
      {selectedEntry && selectedFile ? (
        <div className="dialog-backdrop" role="presentation">
          ...
          >
            ...
            <p className="eyebrow">Confirm document</p>
            <h2 id="confirm-upload-title">Upload this evidence?</h2>
            <p className="dialog-entry">{selectedEntry.eventName}</p>
            ...
            <p className="dialog-note">
              The file will be stored securely against this fixed activity
              entry. A new upload will replace any previous evidence.
            </p>
            <div className="dialog-actions">
              ...
              >
                <UploadIcon />
                {uploadingId ? "Uploading…" : "Confirm upload"}
              </button>
            </div>
          </section>
        </div>
